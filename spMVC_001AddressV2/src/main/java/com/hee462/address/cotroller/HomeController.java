...
public class HomeController {
	 ...
	 */
	// 처음화면에서 주소추가 버튼을 클릭했을때 추가화면을 보여주는 method
	/*
	 * method의 매개변수로 HttpSession 객체를 설정하는 순간
	 * 이 method에서 Session에 저장된 데이터를 참조할수 있다
	 */
	@RequestMapping(value="/insert",method=RequestMethod.GET)
	public String insert(Model model , HttpSession httpSession) {
		
		// Session으로부터 USER Attribute를 getter하고 그데이터를 UserDto type으로 변환하여 userDto 객체에 저장
		// 로그인이 되어 있으면 userDto는 실제 로그인한 user의 정보
		// 로그인이 되어 있지 않으면 userDto 는 null값이다
